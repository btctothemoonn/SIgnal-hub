...
assert.doesNotMatch(source, /\{ value: "1m",/);
assert.match(source, /useState<BinanceHynixPremiumInterval>\("5m"\)/);
assert.match(source, /PREMIUM_INTERVAL_OPTIONS\.some/);
...
assert.doesNotMatch(source, /最近3天/);
...
assert.match(source, /removeBrowserStorage/);
assert.match(source, /STOCKS_HYNIX_PREMIUM_CACHE_KEY\}:1m/);
