...
try {
  ...
  for (const authCode of [401, 403]) {
    ...
    assert.equal(renderer.root.findByProps({ "aria-label": "简报周期" }).findAllByType("button").length, 2);
    assert.doesNotMatch(textContent(renderer.toJSON()), /2 小时/);
    ...
    assert.ok(h.requests.some(({ url }) => url.includes("cadence=daily&limit=10")));
    assert.ok(h.requests.every(({ url }) => !url.includes("cadence=two_hour")));
