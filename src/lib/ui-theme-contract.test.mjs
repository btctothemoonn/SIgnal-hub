...
await test("global theme keeps light and dark semantic tokens internally consistent", () => {
  assert.match(globals, /:root\s*\{[\s\S]*--background:\s*#f3f6f5;/);
  assert.match(globals, /:root\s*\{[\s\S]*--foreground:\s*#1a211e;/);
  assert.match(globals, /:root\s*\{[\s\S]*--accent:\s*#8a651f;/);
  assert.match(globals, /html\.dark\s*\{[\s\S]*--background:\s*#0c0f0e;/);
  assert.match(globals, /html\.dark\s*\{[\s\S]*--foreground:\s*#f5f2eb;/);
