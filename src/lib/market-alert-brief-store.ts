...
function trackItem(item: MarketBriefItem, row: Row | undefined, previous: MarketBriefItem | undefined, nowMs: number) {
  if (nowMs - Date.parse(item.latestAt) > 60 * minutes || !row || row.stale || row.error) return null;
  ...
  // Squeeze enrichment may retain trigger-time 15m/volume/OI values. Use the
  // independently fetched 1m/5m candles for those symbols instead.
  const squeeze = item.squeeze > 0;
  const fast = squeeze ? metrics.pct1m : metrics.pct5m;
  const slow = squeeze ? metrics.pct5m : metrics.pct15m;
  const volume = squeeze ? metrics.volumeRatio1m : metrics.volumeRatio5m;
  if (!finite(fast) || !finite(slow) || !finite(volume) || volume < 0) return null;
  const sign = item.direction === "down" ? -1 : 1;
  // A reversed or fully cooled impulse is not worth occupying a watch slot.
  if (sign*slow <= 0 || sign*fast <= -0.5 || Math.abs(slow) < 0.5 || volume < 0.8) return null;
  ...
  const bands = [momentumBand,volumeBand,slowBand];
  const strength = momentumBand + volumeBand + slowBand;
  ...
  const improving = sameDirection && previousTrack && observed > Date.parse(previousTrack.observedAt) && notWeaker && strength > (previousTrack.strength ?? strength);
  const state = !aligned ? "waiting" : !previousTrack || !sameDirection ? "new"
    : observed === Date.parse(previousTrack.observedAt) ? previousTrack.state
    : improving ? "strengthening" : notWeaker ? "continuing" : "waiting";
  const direction = sign > 0 ? "上涨" : "下跌";
  const periods = squeeze ? "1m / 5m" : "5m / 15m";
  const signed = (value: number) => `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`;
  const evidence = [
    `${periods} 涨跌 ${signed(fast)} / ${signed(slow)}${mixed ? "；窗口内涨跌预警反复" : ""}`,
    `${squeeze ? "1m" : "缓存短周期"} 成交量比 ${volume.toFixed(2)} 倍${!squeeze && finite(metrics.oiGrowth15m) ? `；15m OI ${signed(metrics.oiGrowth15m)}` : ""}；${metrics.spotAvailable ? "有现货数据" : "缺少现货佐证"}`,
  ];
  const nextWatch = `下轮观察 ${periods} 是否保持${direction}，短周期量比是否达到或保持 1.5 倍${mixed ? "，且不再出现反向预警" : ""}。`;
  const dropIf = `较长周期方向反转或幅度低于 0.5%、短周期反向幅度达到 0.5%、量比低于 0.8 倍，或缓存超过 20 分钟 / 最新预警超过 60 分钟时移出。`;
  ...
  const narrativeFacts = [
    ...
    metrics.spotAvailable ? "有现货数据，但不据此推断现货同向" : "缺少现货佐证",
  ];
  const tracking: NonNullable<MarketBriefItem["tracking"]> = {
    state, observedAt:metrics.observedAt,evidence,nextWatch,dropIf,strength,bands,narrativeFacts,
    signalKey:JSON.stringify([item.direction,state,momentumBand,volumeBand,slowBand,aligned,mixed,oiBand,Boolean(metrics.spotAvailable),squeeze,narrativeFacts]),
  };
  const reason = aligned ? "短周期价格与成交量同向，值得继续观察延续性。" : mixed ? "方向出现反复，需等待量价重新一致。" : "仍有方向性变化，但短周期动量或量能尚未确认。";
  // Count is only a final tie-breaker, never the main qualification criterion.
  return { item:{...item,reason,tracking}, rank:(aligned ? 100 : 0)+strength*5+(nowMs-Date.parse(item.latestAt)<=15*minutes ? 3 : 0)+(metrics.spotAvailable ? 1 : 0) };
}
