...
assert.ok(source.indexOf('flock -n 9') < source.indexOf('git pull --ff-only'), "lock must precede all checkout changes");
assert.match(source, /SIGNAL_HUB_DEPLOY_LOCK_HELD=1/);
assert.match(source, /\.release-commit/);
assert.match(source, /exec env SIGNAL_HUB_DEPLOY_REEXEC=1 SIGNAL_HUB_PREVIOUS_COMMIT=/);
...
assert.ok(source.indexOf('activate "$release"') > buildIndex, "activation must follow a successful isolated build");
assert.match(source, /git archive HEAD \| tar -x -C "\$release"/);
assert.match(source, /mv -Tf "\$pending_link" "\$CURRENT_LINK"/);
assert.match(source, /trap rollback ERR/);
assert.match(source, /activate "\$previous_release"/);
assert.match(source, /scripts\/check-deployment\.mjs/);
