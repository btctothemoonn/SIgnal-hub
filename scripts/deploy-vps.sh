...
if ! systemctl list-unit-files --type=service | grep -q '^signal-hub-opportunity.service'; then
  sudo tee /etc/systemd/system/signal-hub-opportunity.service >/dev/null <<EOF
[Unit]
...
EOF
  sudo systemctl daemon-reload
  sudo systemctl enable signal-hub-opportunity
fi
